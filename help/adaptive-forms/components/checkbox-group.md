...
在此範例中，Options元素用於將核取方塊群組在一起。 **顯示文字**&#x200B;專案是用來提供專案的標籤，而&#x200B;**資料值**&#x200B;是用來指定提交表單時傳送至伺服器的值。
...
- **多重選擇**：核取方塊群組可讓使用者從清單中選取多個選項，這在允許或需要多重選擇的情況下很有用。

- **使用者體驗**：核取方塊群組可讓使用者以清楚且直覺的方式選取多個選項，讓表單更方便使用。

- **資料分析**：核取方塊群組可用來收集各種來源的資料，並分析資料，或用來作為進一步處理的輸入。

- **調查**：可在調查中使用核取方塊群組來選取問題的多個選項。

- **使用者偏好設定**：核取方塊群組可用來收集不同選項的使用者偏好設定。

- **資料值**：核取方塊群組也可以用來處理核取方塊群組的專案。

## 版本和相容性 {#version-and-compatibility}

Adaptive Forms核取方塊群組核心元件於2023年2月發行，屬於Cloud Service核心元件2.0.4以及AEM 6.5.16.0 Forms或更新版本的核心元件1.1.12的一部分。 下表顯示所有支援版本、AEM相容性以及對應檔案的連結：
...
在[GitHub](https://github.com/adobe/aem-core-forms-components/tree/master/ui.af.apps/src/main/content/jcr_root/apps/core/fd/components/form/checkboxgroup/v1/checkboxgroup)的技術檔案中取得最適化Forms核取方塊群組核心元件的最新資訊。 如需開發核心元件的詳細資訊，請參閱[核心元件開發人員檔案](/help/developing/overview.md)。
...
- **允許標題使用RTF格式** — 此功能可讓使用者格式化純文字標題，並整合如粗體、斜體、底線文字、各種字型、字型大小、顏色等功能以及可增強視覺呈現和自訂的其他選項。 它提供更大的彈性和創意控制，讓標題在檔案、網站或應用程式中脫穎而出。\
  選取&#x200B;**允許標題為RTF格式**&#x200B;的核取方塊後，即可看到格式化選項，以設定元件標題的樣式。 若要存取所有可用的格式選項，您可以按一下`Fullscreen` ![全熒幕圖示](/help/adaptive-forms/assets/fullscreen-icon.png)標籤。
...
- **選項** — 您可以使用&#x200B;**新增**&#x200B;按鈕來新增資料值並顯示文字組。\
  ...
  您也可以使用&#x200B;**允許選項使用RTF格式**&#x200B;來格式化核取方塊群組的選項。
...
- **顯示選項** — 此選項是用來設定最適化表單中核取方塊的視覺對齊方式。 支援的兩個選項為：
   - **Horizontal** — 選取此選項時，核取方塊會在最適化表單中由左至右顯示。
   - **垂直** — 選取此選項時，核取方塊會以最適化表單由上到下顯示。

- **預設選項** — 此選項可讓您在表單載入時新增預先選取的預設值。 使用刪除圖示可移除新增的選項。 如果提交值&#x200B;**的**&#x200B;資料型別設定為`Number`，而您新增字串資料至&#x200B;**預設選項**，畫面會顯示`Value type mismatch`錯誤訊息。
