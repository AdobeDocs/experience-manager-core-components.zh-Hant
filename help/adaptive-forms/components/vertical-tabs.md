...
- **改善可用性**：垂直標籤讓使用者更容易在表單中導覽，尤其是表單有多個區段或大量欄位時。

- **空間管理**：垂直索引標籤可協助將相關的表單區段分組為索引標籤，一次僅顯示一個區段，節省熒幕空間。

- **更好的組織**：索引標籤為表單提供清楚且有條理的結構，讓使用者更容易瞭解並完成表單。

- **增加使用者參與**：垂直索引標籤可以讓表單更吸引目光、更吸引使用者，進而提高表單完成率。

## 版本和相容性 {#version-and-compatibility}

最適化Forms垂直標籤核心元件已發行作為核心元件2.0.18的一部分。下表顯示所有支援版本、AEM相容性以及對應檔案的連結：
...
您可以透過「設定」對話方塊輕鬆自訂訪客的垂直索引標籤體驗。 您也可以輕鬆定義垂直標籤選項，以提供順暢的使用者體驗。
...
![重複標籤](/help/adaptive-forms/assets/vertical-tab-repeat-vertical-tab.png)

您可以使用重複性選項來複製「垂直定位點」元件及其子元件、定義最小和最大重複計數，以及簡化表單中類似區段的複製。 與「垂直」標籤元件互動並存取其設定時，會顯示下列選項：

- **讓垂直索引標籤可重複**：一種切換功能，可讓使用者啟用或停用重複測量功能。
- **最小重複次數**：建立垂直標籤元件可重複的最小次數。 如果值為零，表示「垂直定位點」元件不會重複；預設值為零。
- **最大重複次數**：設定垂直標籤元件可重複的最大次數。 預設情況下，此值為無限制。

若要有效管理垂直標籤內的可重複區段，請遵循以下提供的步驟： [建立具有可重複區段的表單](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/content/forms/adaptive-forms-authoring/authoring-adaptive-forms-core-components/create-an-adaptive-form-on-forms-cs/create-forms-repeatable-sections.html) 文章。

### 專案標籤 {#items-tab}

![專案索引標籤](/help/adaptive-forms/assets/vertical-tab-items.png)

此 **新增** 按鈕可讓您從元件選取視窗中選取要新增為面板的元件。 新增元件後，您會看到下列選項：

- **圖示**  — 圖示可識別清單中面板的元件。 您可以將滑鼠停留在圖示上，以工具提示的形式檢視完整的元件名稱。
- **說明**  — 面板文字所用的說明。 依預設，為面板選取的元件名稱。
- **刪除**  — 點選或按一下以從垂直標籤元件中刪除面板。
- **重新排列**  — 點選或按一下並拖曳以重新排列面板。

### 說明內容標籤 {#help-content}

![說明內容標籤](/help/adaptive-forms/assets/vertical-tab-help.png)
...
![協助工具標籤](/help/adaptive-forms/assets/vertical-tab-accessibility.png)
...
「設計」對話方塊可讓範本建立者控制物件的預設顯示方式。 針對最適化Forms垂直標籤元件，您可以設定下列專案：

- 表單建立者可以在最適化Forms編輯器中新增到垂直索引標籤的核心元件
- 樣式（CSS類別）的簡單名稱，可套用至最適化Forms編輯器中Vertical-tab元件的屬性對話方塊中。
...
此 **允許的元件** 索引標籤可讓範本編輯器設定元件，這些元件可在最適化Forms編輯器的垂直索引標籤元件中新增為面板的專案。

### 樣式索引標籤 {#styles-tab}

![樣式索引標籤](/help/adaptive-forms/assets/tabs-styles-tab.png)

「設計」對話方塊可用來定義和管理元件的CSS樣式。 最適化Forms垂直標籤核心元件支援AEM [樣式系統](/help/get-started/authoring.md#component-styling).

- **預設CSS類別**：您可以為最適化Forms垂直索引標籤核心元件提供預設CSS類別。
