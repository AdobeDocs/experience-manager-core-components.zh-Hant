...
最適化Forms核心元件使用語意`<fieldset>`和`<legend>` HTML元素來改善選項按鈕群組的協助工具。 這些元素可讓熒幕助讀程式精確解譯表單結構，並傳達群組標籤與其選項之間的關係。

選項按鈕(v2)元件會在`<fieldset>`元素中轉譯，群組標籤會包裝在`<legend>`中，作為其第一個子系。 多餘或無效的ARIA屬性會被移除，且個別標籤會正確與其各自的輸入建立關聯。 這可確保熒幕助讀程式可清楚讀取每個選項按鈕標籤，以及其群組內容。
