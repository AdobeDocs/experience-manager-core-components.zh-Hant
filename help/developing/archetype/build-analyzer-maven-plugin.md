...
如需如 [何將Maven Plugin納入AEM Maven專案的詳細資訊](https://github.com/adobe/aemanalyser-maven-plugin/blob/main/aemanalyser-maven-plugin/README.md) ，請參閱Maven Plugin檔案。
...
| `api-regions`<p> </p>`api-regions-check-order`<p> </p>`api-regions-dependencies`<p> </p>`api-regions-duplicates` | 這些分析器會檢查與內容套件 [至特徵模型轉換程式相關的詳細資訊](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/deploying/overview.html?lang=en#deploying) ，以建立符合Sling Feature Model的假象。 任何錯誤都應向Adobe客戶支援報告。 | 是 | 是 |
