...
* **新專案**
新專案應一律嘗試使用核心元件。 如果核心元件無法直接使用或[延伸](customizing.md)以滿足專案需求，請遵循核心元件中所述的元件架構建立自訂元件。 除非沒有其他方法，否則請避免使用[foundation元件](/help/versions.md#foundation-component-support)。
* **現有的專案**
建議持續使用[foundation元件](/help/versions.md#foundation-component-support)，除非已計畫網站或元件重構。\
  由於大部分現有專案已廣泛使用基礎元件，因此將繼續支援基礎元件[。](/help/versions.md#foundation-component-support)
* **新自訂元件**
評估是否可以自訂現有的[核心元件](customizing.md)。\
  如果不適用，建議依照[元件指導方針](guidelines.md)建置新的自訂元件。
* **現有的自訂元件**
如果您的元件如預期般運作，則請原樣保留。\
  否則，請參閱上方的「新自訂元件」。
...
如需AEMCS專案的詳細資訊，請參閱檔案[AEMaaCS專案結構](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/developing/aem-project-content-package-structure.html?lang=zh-Hant)。
...
| 貢獻 | 透過提取請求 | 不可能 |
| 親和力 | 完全符合[WCAG 2.0 AA Standard](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/sites/authoring/fundamentals/accessible-content.html?lang=zh-Hant) | 僅部分符合[WCAG 2.0 AA標準](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/sites/authoring/fundamentals/accessible-content.html?lang=zh-Hant) |
...
| 核心元件 | 說明 | 取代的基礎元件 |
|---|---|---|
| [Page](https://adobe.com/go/aem_cmp_tech_page_v2_tw) | 使用範本編輯器的回應式頁面 | `/libs/foundation/components/page /libs/wcm/foundation/components/page` |
| [階層連結](https://adobe.com/go/aem_cmp_tech_breadcrumb_v2_tw) | 頁面階層導覽 | `/libs/foundation/components/breadcrumb` |
| [標題](https://adobe.com/go/aem_cmp_tech_title_v2_tw) | H1-H6標題 | `/libs/foundation/components/title /libs/wcm/foundation/components/title` |
...
| [影像](https://adobe.com/go/aem_cmp_tech_image_v2_tw) | 以智慧和延遲的方式載入最佳轉譯大小 | `/libs/foundation/components/image /libs/foundation/components/adaptiveimage /libs/foundation/components/logo /libs/foundation/components/mobileimage  /libs/foundation/components/mobilelogo /libs/wcm/foundation/components/image` |
| [清單](https://adobe.com/go/aem_cmp_tech_list_v2_tw) | 頁面清單 | `/libs/foundation/components/list /libs/foundation/components/mobilelist /libs/wcm/foundation/components/list` |
| [社交媒體分享](https://adobe.com/go/aem_cmp_tech_sharing_v1_tw) | facebook和Pinterest共用Widget | `-` |
| [來自容器](https://adobe.com/go/aem_cmp_tech_form_container_v2_tw) | 回應式表單段落系統 | `/libs/foundation/components/form/start /libs/foundation/components/form/end` |
...
| [已隱藏的表單](https://adobe.com/go/aem_cmp_tech_form_hidden_v2_tw) | 隱藏輸入欄位 | `/libs/foundation/components/form/hidden` |
...
| [語言導覽](https://adobe.com/go/aem_cmp_tech_langnav_v1_tw) | 列出全球語言結構的語言和國家/地區切換器 | `-` |
| [快速搜尋](https://adobe.com/go/aem_cmp_tech_search_v1_tw) | 在下拉式選單中將結果顯示為就地建議的搜尋元件 | `/libs/foundation/components/search` |
| [Teaser](https://adobe.com/go/aem_cmp_tech_teaser_v1_tw) | 允許內容作者輕鬆建立Teaser，以使用影像、標題或RTF文字進一步內容並連結至進一步內容或其他動作 | `-` |
| [索引標籤](https://adobe.com/go/aem_cmp_tech_tabs_v1_tw) | 允許內容作者將頁面內容整理在多個索引標籤中 | `-` |
| [傳送](https://adobe.com/go/aem_cmp_tech_carousel_v1_tw) | 允許內容作者將內容整理在旋轉的幻燈片輪播中 | `/libs/foundation/components/carousel` |
...
| [折疊式面板](https://adobe.com/go/aem_cmp_tech_accordion_v1_tw) | 在可摺疊的摺疊式功能表中整理內容面板 | `-` |
...
| [內嵌](https://adobe.com/go/aem_cmp_tech_embed_v1_tw) | 在頁面中內嵌外部資源 | - |
| [進度列](https://adobe.com/go/aem_cmp_tech_progress_v1) | 提供目標進度的視覺化表示 | - |
| [PDF檢視器](https://adobe.com/go/aem_cmp_tech_pdfviewer_v1_tw) | 在頁面上顯示PDF檔案 | - |

## 核心元件的升級 {#upgrade-of-core-components}

版本化元件的一個優點是，它允許將移轉至新AEM版本與移轉至新元件版本分開。 此外，如果有新的元件版本，可讓個別元件移轉至新版本。

移轉至新AEM版本不會影響核心元件的運作方式，前提是其版本也支援正在移轉至的新AEM版本。 對核心元件進行的自訂作業也不應受影響，只要這些自訂作業未使用已[棄用或移除的API](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/release-notes/deprecated-removed-features.html?lang=zh-Hant)。
