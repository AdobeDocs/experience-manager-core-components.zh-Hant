...
>[!TIP]
>
>請參閱 [適用性影像Servlet](#adaptive-image-servlet) 如需這些功能的詳細技術資訊，以及最佳化轉譯選取的秘訣。
...
>[!CAUTION]
>
>SVG支援需要2.1.0版或更新版本的核心元件，以及 [服務包2](https://experienceleague.adobe.com/docs/experience-manager-64/release-notes/sp-release-notes.html?lang=zh-Hant) AEM 6.4或更新版本，以支援 [影像編輯器功能](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/implementing/components-templates/image-editor.html) 在AEM中。
...
>[!TIP]
>
>請參閱 [適用性影像Servlet](#adaptive-image-servlet) 如需其功能的詳細技術資訊，以及仔細定義寬度以最佳化轉譯選取的秘訣。
...
影像元件使用核心元件的最適化影像Servlet。 [最適化影像Servlet](https://github.com/adobe/aem-core-wcm-components/wiki/The-Adaptive-Image-Servlet) 負責影像處理和串流，開發人員可在 [核心元件的自訂](/help/developing/customizing.md).

### 最佳化轉譯選取項目 {#optimizing-rendition-selection}

適用性影像Servlet將嘗試為請求的影像大小和類型挑選最佳的轉譯。 建議您在同步中定義DAM轉譯和影像元件允許的寬度，讓適用性影像Servlet盡可能少地執行處理作業。

這可改善效能，並避免基礎影像處理程式庫無法正確處理某些影像。

>[!NOTE]
>
>透過 `Last-Modified` 適用性影像Servlet支援標題，但快取 `Last-Modified` 標題 [需要在Dispatcher中啟用](https://experienceleague.adobe.com/docs/experience-manager-dispatcher/using/configuring/dispatcher-configuration.html?lang=en#caching-http-response-headers).
>
